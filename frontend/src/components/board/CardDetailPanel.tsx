    ...
    <>
      ...
      <div className="absolute top-0 right-0 bottom-0 w-[420px] bg-surface border-l border-border flex flex-col z-40 animate-[fade-up_0.2s_ease-out] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <span className="text-xs font-semibold uppercase tracking-wider text-text-dim">Card</span>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          <input
            ...
            className="w-full text-base font-medium bg-transparent outline-none border border-transparent focus-visible:border-accent focus-visible:bg-surface-2 rounded-md -mx-1 px-1 py-0.5"
          />
          <textarea
            value={description}
            ...
            rows={5}
            className="w-full text-sm text-text-muted bg-surface-2 border border-border rounded-lg p-3 outline-none resize-none focus-visible:border-accent"
          />
        </div>
